...
/*
JJD ==> Modification to migrate on an ESP32 with wifi, no RF. 
*/
...
// JJD test on 1 phase #define NO_OF_PHASES 3
...
// JJD const byte loadPrioritySelectorPin = 3; // // for 3-phase PCB
const byte loadPrioritySelectorPin = 1; // // for 1-phase PCB
...
//
// JJD const float powerCal[NO_OF_PHASES] = {0.043, 0.043, 0.043};

      const float powerCal[NO_OF_PHASES] = {0.043};              // 1 phase    
...
//
//JJD const float  phaseCal[NO_OF_PHASES] = {0.5, 0.5, 0.5}; // <- nominal values only

const float  phaseCal[NO_OF_PHASES] = {0.5}; // <- nominal values only 1 phase
...
// JJ {
  ...
  //JJD }  // <-- this closing brace needs to be outside the WORKLOAD_CHECK blocks! 
...
{  
  ...
  // jjd Serial.print(">>free RAM = ");
  // jjd Serial.println(freeRam());  // a useful value to keep an eye on
}
